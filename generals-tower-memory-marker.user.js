...
;(() => {
  ...
  function 挂钩socket(socket) {
    ...
    function 处理基地位置(数据包, 来源事件) {
      ...
      const 基地列表 = Array.isArray(数据包 && 数据包.generals)
        ? 数据包.generals
        : null
    ...
    function 重置本局(数据包) {
      ...
      状态.我方索引 = Number.isInteger(数据包 && 数据包.playerIndex)
        ? 数据包.playerIndex
        : null
      状态.队伍 = Array.isArray(数据包 && 数据包.teams)
        ? 数据包.teams.slice()
        : null
      尝试从地图读取尺寸(数据包 || {})
    ...
    function 更新地图缓存和兵力分布(数据包, 来源事件) {
      ...
      if (完整地图数组) {
        ...
        Array.isArray(数据包 && 数据包.map_diff) &&
      ...
      function 取得不拆相邻同兵力地块列表(地块列表) {
        ...
        for (const 地块 of 地块列表) {
          if (!待访问地块.has(地块.索引)) continue
          const 同兵力相邻地块 = 取得同兵力相邻地块(地块)
          if (着色列表.length + 同兵力相邻地块.length > 兵力着色最多数量) {
            超限同兵力跳过数量 += 同兵力相邻地块.length
            break
          }
          着色列表.push(...同兵力相邻地块)
        }
        ...
        function 取得同兵力相邻地块(起始地块) {
          const 结果 = []
          const 队列 = [起始地块]
          待访问地块.delete(起始地块.索引)

          for (let i = 0; i < 队列.length; i += 1) {
            const 当前地块 = 队列[i]
            结果.push(当前地块)
            取得相邻索引(当前地块.索引).forEach((相邻索引) => {
              const 相邻地块 = 待访问地块.get(相邻索引)
              if (!相邻地块 || 相邻地块.兵力 !== 起始地块.兵力) return
              待访问地块.delete(相邻索引)
              队列.push(相邻地块)
            })
          }
  ...
  function 渲染() {
    ...
    function 画兵力分布着色() {
      ...
      const 内缩 = Math.max(1, 大小 * 0.02)
      const 高兵力阈值 = 最低兵力 + 兵力跨度 * 0.7
