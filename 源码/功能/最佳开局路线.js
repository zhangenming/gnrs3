...
function 处理开局路线地图更新(数据包, 来源事件) {
  if (来源事件 !== '新局重置' && 来源事件 !== 'game_update') return
...
function 计算最佳开局路线(数据包) {
  ...
  const 最优不足计划列表 = 开局模板列表
    .map((模板) => 搜索模板最优不足路线(模板, 取得搜索上下文()))
    .filter(Boolean)
  if (最优不足计划列表.length) {
    最优不足计划列表.sort((左, 右) => {
      if (右.陆地数 !== 左.陆地数) return 右.陆地数 - 左.陆地数
      if (右.评分 !== 左.评分) return 右.评分 - 左.评分
      return 左.模板顺序 - 右.模板顺序
    })
    return {
...
function 搜索普通开局路线(上下文) {
  const 模板 = 开局模板列表[0]
  const 基地邻居列表 = 取得相邻索引列表(上下文.基地索引)
    .filter((索引) => {
      return 是开局可走地(索引, 上下文)
    })
    .sort((左, 右) => {
      return 计算普通开局方向分数(右) - 计算普通开局方向分数(左)
    })
  if (基地邻居列表.length < 3) return null

  for (const 第一方向 of 基地邻居列表) {
    for (const 第三方向 of 基地邻居列表) {
      if (第三方向 === 第一方向) continue
      for (const 第四方向 of 基地邻居列表) {
        if (第四方向 === 第一方向 || 第四方向 === 第三方向) continue

        const 保留直开方向集合 = new Set([第三方向, 第四方向])
        const 第一状态 = 尝试追加分支段(
          { 已占集合: new Set([上下文.基地索引]), 段列表: [] },
          模板.段列表[0],
          [上下文.基地索引],
          第一方向,
          保留直开方向集合,
        ...
        for (const 第二自有路径 of 第二自有路径列表) {
          ...
          if (!第四状态 || 第四状态.已占集合.size < 目标陆地数) continue

          const 方向数 = 取得开局方向数(第四状态.已占集合, 上下文.基地索引)
          return {
            已找到: true,
            记法: 模板.记法,
            描述: 模板.描述,
            陆地数: 第四状态.已占集合.size,
            段列表: 第四状态.段列表,
            评分: 第四状态.已占集合.size * 10000 + 方向数 * 600,
            模板顺序: 0,
          }
        }
      }
  ...
  function 尝试追加分支段(路线状态, 段, 自有路径, 必须首格, 保留集合) {
    const 起点 = 自有路径.at(-1)
    const 新地路径 = 取得分支新地路径(
      起点,
      路线状态.已占集合,
    ...
    const 下个已占集合 = new Set(路线状态.已占集合)
    新地路径.forEach((索引) => {
      下个已占集合.add(索引)
    })
    return {
      ...
      段列表: [
        ...路线状态.段列表,
        {
          ...
          自有路径,
          新地路径,
          方向文本: 取得分支段方向文本(段, 自有路径, 新地路径),
        },
      ],
    }
  }

  function 取得分支新地路径(起点, 已占集合, 新地数, 必须首格, 保留集合) {
    const 新地路径 = []
    const 新地集合 = new Set()
    const 候选集合 = new Set()

    if (Number.isInteger(必须首格)) {
      添加候选(必须首格)
    } else {
      取得相邻索引列表(起点).forEach(添加候选)
    }

    while (新地路径.length < 新地数) {
      if (!候选集合.size) return null
      const 候选 = Array.from(候选集合).sort((左, 右) => {
        return 计算分支候选分数(右) - 计算分支候选分数(左)
      })[0]
      候选集合.delete(候选)
      if (!可加入新地(候选)) continue

      新地路径.push(候选)
      新地集合.add(候选)
      取得相邻索引列表(候选).forEach(添加候选)
    }
    return 新地路径
    ...
    function 可加入新地(索引) {
      if (已占集合.has(索引)) return false
      if (新地集合.has(索引)) return false
      if (保留集合.has(索引)) return false
      return 是开局可走地(索引, 上下文)
    }

    function 计算分支候选分数(索引) {
      let 开口数 = 0
      for (const 邻居 of 取得相邻索引列表(索引)) {
        if (已占集合.has(邻居) || 新地集合.has(邻居)) continue
        if (保留集合.has(邻居)) continue
        if (是开局可走地(邻居, 上下文)) 开口数 += 1
      }
      return 开口数 * 100 + 取得曼哈顿距离(索引, 上下文.基地索引)
    }
  }

  function 取得分支段方向文本(段, 自有路径, 新地路径) {
    const 方向起点 = 自有路径[1] ?? 新地路径[0] ?? 自有路径[0]
    const 方向文本 = 取得方向文本(上下文.基地索引, 方向起点) || '支路'
    if (段.自有步数) {
      return `${方向文本}走${段.自有步数}再开${段.新地数}地`
    }
    return `${方向文本}开${段.新地数}地`
  }

  function 计算普通开局方向分数(索引) {
    const 路径 = 取得分支新地路径(
      上下文.基地索引,
      new Set([上下文.基地索引]),
      Math.min(模板.段列表[0].新地数, 目标陆地数 - 1),
      索引,
      new Set(),
    )
    return (
      (路径?.length ?? 0) * 100 + 取得开口数(索引, new Set(), new Set(), 上下文)
    )
  }
}

function 搜索模板最优不足路线(模板, 上下文) {
  let 最佳结果 = null
  搜索段(0, new Set([上下文.基地索引]), [])
  if (!最佳结果) return null

  const 模板顺序 = 开局模板列表.indexOf(模板)
  const 方向数 = 取得开局方向数(最佳结果.最终已占集合, 上下文.基地索引)
  const 回走步数 = 最佳结果.段列表.reduce((合计, 段) => {
    return 合计 + 段.自有步数
  }, 0)
  const 评分 =
    最佳结果.最终已占集合.size * 10000 +
    方向数 * 600 -
    回走步数 * 35 -
    最佳结果.段列表.length * 12 -
    模板顺序
  ...
  function 搜索段(段idx, 当前已占集合, 当前段列表) {
    ...
    for (const 自有路径 of 自有路径列表) {
      if (搜索已超时(上下文)) return
      const 起点 = 自有路径.at(-1)
      const 新地路径列表 = 取得不足新地路径候选(起点, 当前已占集合, 段.新地数)
  ...
  function 更新最佳结果(当前已占集合, 当前段列表) {
    if (!当前段列表.length) return
    if (当前已占集合.size >= 目标陆地数) return
    const 当前方向数 = 取得开局方向数(当前已占集合, 上下文.基地索引)
    const 当前评分 = 当前已占集合.size * 10000 + 当前方向数 * 600
    const 最佳评分 =
      (最佳结果?.最终已占集合.size ?? 0) * 10000 +
      取得开局方向数(最佳结果?.最终已占集合 ?? new Set(), 上下文.基地索引) * 600
    if (当前评分 <= 最佳评分) return

    最佳结果 = {
      最终已占集合: new Set(当前已占集合),
      段列表: 当前段列表,
    }
  }

  function 取得不足新地路径候选(起点, 已占集合, 最大新地数) {
    const 候选列表 = []
    const 每档候选上限 = Math.max(2, Math.floor(新地路径候选上限 / 最大新地数))
    for (let 新地数 = 最大新地数; 新地数 >= 1; 新地数 -= 1) {
      const 当前候选列表 = 取得新地路径候选(起点, 已占集合, 新地数, 上下文)
      候选列表.push(...当前候选列表.slice(0, 每档候选上限))
      if (候选列表.length >= 新地路径候选上限) break
    }
    return 候选列表
  }
