...
import { 统计塔数, 补齐未知开塔归属 } from './塔数统计.js'
...
let 请求回放塔信息渲染 = null
let 回放塔信息缓存 = null
const 回放敌方开塔同步版本 = '只记录官方明确中立转敌方-v3'
...
export function 同步网页回放塔数据() {
  ...
  const 旧回放地图数组 = 是连续回合 ? 旧缓存.地图数组 : null
  const 需要重置塔数据 = !是同回合 && !是连续回合
  const 需要清理回放敌方开塔 =
    状态.敌方开塔回放同步版本 !== 回放敌方开塔同步版本
  ...
  if (需要重置塔数据) 清空回放塔数据()
  if (需要重置塔数据 || 需要清理回放敌方开塔) {
    清理回放敌方开塔旧推断()
  }
  回放数据包.gio跳过敌方开塔归属记录 = true
  处理塔位置(回放数据包, 请求回放塔重绘)
  记录回放明确敌方开塔()
  补齐未知开塔归属()
  状态.敌方开塔回放同步版本 = 回放敌方开塔同步版本
  ...
  function 记录回放明确敌方开塔() {
    if (!地图可读(旧回放地图数组)) return
    if (!Array.isArray(回放数据包.cities)) return

    for (const 塔索引 of 回放数据包.cities) {
      if (!Number.isInteger(塔索引) || 塔索引 < 0) continue

      const 旧归属 = 读取地图归属(旧回放地图数组, 塔索引)
      const 新归属 = 读取地图归属(回放地图数组, 塔索引)
      if (旧归属 !== -1) continue
      if (!Number.isInteger(新归属) || 新归属 < 0) continue
      if (是我方或队友(新归属)) continue
