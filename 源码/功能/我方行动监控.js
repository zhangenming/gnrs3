  ...
  function 同步网页回放行动数据() {
    ...
    function 记录回放官方行动(起始回合, 结束回合, 起始地图数组, 数据包) {
      ...
      function 删除回合开塔兵力(回合) {
        状态.行动开塔兵力表.delete(取得行动开塔兵力键('我方', 回合))
        状态.行动开塔兵力表.delete(取得行动开塔兵力键('敌方', 回合))
      }

      function 记录回放开塔兵力(回合, 移动, 地图数组, 阵营, 行动类型) {
        if (行动类型 !== '偷塔' && 行动类型 !== '开塔') return

        const 开塔兵力 = 取得回放开塔兵力(移动, 地图数组)
        设置行动开塔兵力(阵营, 回合, 开塔兵力)

        function 取得回放开塔兵力(移动, 地图数组) {
...
function 设置行动开塔兵力(阵营, 回合, 开塔兵力) {
  if (!Number.isInteger(回合) || 回合 < 取得行动监控起始回合()) return
  if (!Number.isInteger(开塔兵力) || 开塔兵力 <= 0) return

  const 键 = 取得行动开塔兵力键(阵营, 回合)
  const 旧兵力 = 状态.行动开塔兵力表.get(键)
  if (Number.isInteger(旧兵力) && 旧兵力 >= 开塔兵力) return

  状态.行动开塔兵力表.set(键, 开塔兵力)
  我方行动监控数据版本 += 1
  更新我方行动监控UI()
}
