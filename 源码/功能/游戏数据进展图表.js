...
function 读取开塔进展(回合) {
  ...
  function 读取敌方开塔记录(当前回合) {
    const 调试 = 状态.敌方开塔调试
    if (调试?.回合 === 当前回合) {
      const 是确认开塔 = Boolean(调试.是敌方成功开塔 || 调试.新增开塔数 > 0)
      return {
        开塔兵力列表:
          是确认开塔 && 调试.敌方偷塔耗兵 > 0 ? [调试.敌方偷塔耗兵] : [],
        开塔成功: 是确认开塔,
        开塔确认: 是确认开塔,
      }
    }
    ...
    return {
      ...
      开塔成功: Boolean(旧记录?.敌方开塔确认 && 旧记录?.敌方开塔成功),
      开塔确认: Boolean(旧记录?.敌方开塔确认),
    }
  }
}
...
function 读取开塔兵力列表(数据点, 玩家) {
  if (!数据点) return []
  if (玩家 === '敌方' && 数据点.敌方开塔确认 !== true) return []
...
function 确保开塔进展算法版本() {
  if (状态.游戏数据进展开塔算法版本 === 开塔进展算法版本) return

  for (const 数据点 of 状态.游戏数据进展列表) {
    delete 数据点.敌方开塔兵力
    delete 数据点.敌方开塔兵力列表
    delete 数据点.敌方开塔成功
    delete 数据点.敌方开塔确认
  }
  状态.游戏数据进展开塔算法版本 = 开塔进展算法版本
  图表渲染签名表.clear()
}
