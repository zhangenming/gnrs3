...
export function 取得选中棋子索引() {
  ...
  const 当前移动队列最新索引 = 取得移动队列最新索引()
  if (
    当前移动队列长度 > 已同步移动队列长度 ||
    当前移动队列最新索引 !== 已同步移动队列最新索引
  ) {
    同步移动队列当前位置选中()
  }
...
function 同步移动队列当前位置选中() {
  const 当前移动位置 = 取得移动队列最新索引()
  if (Number.isInteger(当前移动位置)) {
    选中格子索引 = 当前移动位置
  }
  同步移动队列标记()
}
...
function 取得移动队列最新索引() {
  ...
  return Number.isInteger(最后移动?.终点) && 最后移动.终点 >= 0
    ? 最后移动.终点
    : null
}
