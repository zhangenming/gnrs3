...
function 画塔标记(ctx, x, y, 大小, 类型, 当前动画时间, 塔索引) {
  const 是敌方塔 = 类型 === '敌方塔'
  const 是我方塔 = 类型 === '我方塔'
  const 是已占领塔 = 是敌方塔 || 是我方塔
  const 是基地变成的塔 = 状态.已知基地集合.has(塔索引)
...
export function 更新塔类型(数据包, 塔索引) {
  ...
  function 记录开塔归属(旧类型, 可见类型) {
    if (旧类型 === 可见类型) return
    if (旧类型 === '中立塔' && 可见类型 === '我方塔') {
      状态.我方开塔集合.add(塔索引)
      return
    }
    if (数据包?.gio跳过敌方开塔归属记录) return
    if (旧类型 === '中立塔' && 可见类型 === '敌方塔') {
      状态.敌方开塔确认集合.add(塔索引)
    }
  }
  ...
  function 读取当前塔归属() {
    const 数据包归属 = 读取可见地块归属(数据包, 塔索引)
    if (数据包归属 != null) return 数据包归属
    return 读取地图归属(状态.地图数组, 塔索引)
  }
}
