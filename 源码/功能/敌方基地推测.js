...
//
// 作用范围:
// 只使用当前地图缓存、已到达视野、已知障碍物、塔和基地记忆，维护候选格和排除遮罩。
...
export function 更新敌方基地推测(数据包, 请求渲染) {
  ...
  const 距离上限 = 取得距离上限()
  const 目标距离 = 取得目标距离()
  const 排除距离列表 = 取得接触距离列表(地图信息, 距离上限)
  const 候选距离列表 = 取得接触距离列表(地图信息, 目标距离)
  状态.敌方基地排除集合 = 取得排除集合(地图信息, 排除距离列表)
  状态.敌方基地候选列表 = 取得候选列表(地图信息, 候选距离列表)
  ...
  function 取得接触距离列表(地图信息, 最大距离) {
    if (!Number.isInteger(最大距离) || 最大距离 < 0) return []

    return 状态.敌方基地接触列表
      .map((接触) => 取得距离表(接触, 地图信息, 最大距离))
      .filter(Boolean)
  }

  function 取得排除集合(地图信息, 距离列表) {
    const 排除集合 = new Set()
    if (!距离列表.length || 状态.已知敌方基地集合.size) return 排除集合

    for (let idx = 0; idx < 地图信息.格子数; idx += 1) {
      if (状态.已到达视野集合.has(idx)) continue
      if (任一接触超过距离上限(idx)) 排除集合.add(idx)
    }

    return 排除集合

    function 任一接触超过距离上限(idx) {
      for (const 距离表 of 距离列表) {
        const 距离 = 距离表[idx]
        if (!Number.isInteger(距离) || 距离 < 0) return true
      }
      return false
    }
  }
  ...
  function 取得推测调试(当前回合) {
    ...
    const 距离上限 = 取得距离上限()
    return {
      原因: 取得调试原因(首次接敌敌方陆地, 当前回合),
      当前回合,
      接触数量: 状态.敌方基地接触列表.length,
      排除遮罩数量: 状态.敌方基地排除集合.size,
  ...
  function 取得调试原因(首次接敌敌方陆地, 当前回合) {
    if (状态.已知敌方基地集合.size) return '已发现敌方基地'
    if (!状态.敌方基地接触列表.length) {
      return 当前回合 > 最大首次接敌回合
        ? '超过15回合未记录首次接敌'
        : '尚未接敌'
    }
  ...
  function 取得距离上限() {
    const 首次接敌敌方陆地 = 取得首次接敌敌方陆地()
    if (!Number.isInteger(首次接敌敌方陆地) || 首次接敌敌方陆地 < 1) {
      return null
    }
    return 首次接敌敌方陆地
  }
