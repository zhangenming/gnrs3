...
// 剩下的不明原因变多只作为可疑耗兵，还必须结合双方行动类型和陆地变化确认。
//
// 作用范围:
...
import { 同步网页回放塔数据 } from './战场塔信息.js'
...
export function 更新敌方开塔提示(数据包) {
  ...
  const 行动上下文 = 取得敌方开塔行动上下文(上次快照, 当前快照, 判断)
  const 候选结果 = 更新敌方开塔候选(当前快照, 判断, 行动上下文)
  const 敌方开塔提示耗兵 = 取得敌方开塔提示耗兵(判断, 候选结果)
  const 推断结果 = 是网页回放中()
    ? 跳过回放敌方开塔推断()
    : 应用敌方成功开塔推断(上次快照, 当前快照, 候选结果)
...
function 同步敌方开塔行动监控(当前快照, 敌方开塔提示耗兵, 候选结果) {
  const 回合 = Number.isInteger(候选结果.开塔行动回合)
    ? 候选结果.开塔行动回合
    : 当前快照.回合 - 1
...
function 取得敌方开塔提示耗兵(判断, 候选结果) {
  return Number.isInteger(候选结果.敌方偷塔耗兵) && 候选结果.敌方偷塔耗兵 > 0
    ? 候选结果.敌方偷塔耗兵
    : 判断.敌方偷塔耗兵
}
...
export function 取得敌方开塔判断(上次快照, 当前快照) {
...
function 取得敌方开塔行动上下文(上次快照, 当前快照, 判断) {
  const 起始行动回合 = 上次快照.回合
  const 结束行动回合 = 当前快照.回合 - 1
  const 敌方行动类型列表 = []
  const 我方行动类型列表 = []
  const 敌方开塔兵力列表 = []
  const 我方开塔兵力列表 = []
  ...
  for (let 回合 = 起始行动回合; 回合 <= 结束行动回合; 回合 += 1) {
    const 敌方行动类型 = 状态.敌方行动类型表.get(回合)
    const 我方行动类型 = 状态.我方行动类型表.get(回合)
    if (敌方行动类型) 敌方行动类型列表.push(敌方行动类型)
    if (我方行动类型) 我方行动类型列表.push(我方行动类型)

    const 敌方开塔兵力 = 读取行动开塔兵力('敌方', 回合)
    const 我方开塔兵力 = 读取行动开塔兵力('我方', 回合)
    if (Number.isInteger(敌方开塔兵力)) 敌方开塔兵力列表.push(敌方开塔兵力)
    if (Number.isInteger(我方开塔兵力)) 我方开塔兵力列表.push(我方开塔兵力)
    敌方开塔行动成功 ||= 读取行动开塔成功('敌方', 回合)
    我方开塔行动成功 ||= 读取行动开塔成功('我方', 回合)
  }

  const 敌方行动类型 = 取得最高行动类型(敌方行动类型列表, [
    '集兵',
    '扩地',
    '吃地',
    '开塔',
  ])
  const 我方行动类型 = 取得最高行动类型(我方行动类型列表, [
    '空闲',
    '集兵',
    '扩地',
    '偷塔',
  ...
  const 有行动数据 = 敌方行动类型列表.length > 0 || 我方行动类型列表.length > 0
  const 有敌方开塔行动 =
    敌方行动类型列表.includes('开塔') || 敌方开塔兵力列表.length > 0
  const 有敌方扩地行动 = 敌方行动类型列表.includes('扩地')
  const 有敌方吃地行动 = 敌方行动类型列表.includes('吃地')
  const 有我方抢塔行动 = 我方行动类型列表.includes('吃地(抢塔)')
  const 有我方偷塔行动 =
    我方行动类型列表.includes('偷塔') || 我方开塔兵力列表.length > 0
  const 我方偷塔未扣除 = 有我方偷塔行动 && 判断.我方开塔耗兵 <= 0
  const 有玩家战斗 = 有敌方吃地行动 || 有我方抢塔行动
  const 是可疑敌方耗兵 = 判断.是敌方偷塔攻击 && 判断.敌方变化.净损失 > 0
  const 攻击排除原因 = 取得攻击排除原因()
  const 是敌方偷塔攻击 = !攻击排除原因
  const 成功排除原因 = 取得成功排除原因()
  const 是敌方成功开塔信号 = !成功排除原因
  const 成功开塔数 = 是敌方成功开塔信号
    ? Math.max(1, Math.min(判断.敌方变化.陆地变化, 取得敌方开塔攻击次数()))
    : 0
  ...
  function 取得攻击排除原因() {
    if (!是可疑敌方耗兵) return '兵力差没有未解释的敌方耗兵'
    if (!有行动数据) return '缺少双方行动类型'
    if (有玩家战斗) return '双方交战会改变兵力差'
    if (我方偷塔未扣除) return '我方偷塔耗兵未扣除'
    return null
  }

  function 取得成功排除原因() {
    if (判断.敌方变化.陆地变化 <= 0) return '敌方陆地没有增加'
    if (!有行动数据) return '缺少双方行动类型'
    if (有玩家战斗) return '双方交战会改变兵力差'
    if (我方偷塔未扣除) return '我方偷塔耗兵未扣除'
    if (!有敌方开塔行动 && !有敌方扩地行动) return '敌方行动不是开塔或扩地'
    return null
  }

  function 取得敌方开塔攻击次数() {
    return Math.max(1, 敌方开塔兵力列表.length)
  }

  function 读取行动开塔兵力(阵营, 回合) {
    const 开塔兵力 = 状态.行动开塔兵力表.get(`${阵营}:${回合}`)
    return Number.isInteger(开塔兵力) && 开塔兵力 > 0 ? 开塔兵力 : null
  }

  function 读取行动开塔成功(阵营, 回合) {
    return 状态.行动开塔成功集合.has(`${阵营}:${回合}`)
  }

  function 取得最高行动类型(行动类型列表, 优先级列表) {
    let 最高行动类型 = null
    let 最高优先级 = -1
    for (const 行动类型 of 行动类型列表) {
      const 优先级 = 优先级列表.indexOf(行动类型)
      if (优先级 > 最高优先级) {
        最高行动类型 = 行动类型
        最高优先级 = 优先级
      }
...
function 更新敌方开塔候选(当前快照, 判断, 行动上下文) {
  const 旧候选 = 取得有效敌方开塔候选(当前快照.回合)
  const 本次候选 = 行动上下文.是敌方偷塔攻击
    ? 合并敌方开塔候选(旧候选, 当前快照, 判断)
    : 旧候选
  const 是敌方成功开塔 = Boolean(本次候选) && 行动上下文.是敌方成功开塔
  const 成功开塔数 = 是敌方成功开塔
    ? Math.max(1, Math.min(行动上下文.成功开塔数, 本次候选.攻击次数))
    : 0
  ...
  return {
    ...
    当前候选,
    敌方偷塔耗兵: 本次候选?.累计耗兵 ?? 0,
    开塔行动回合: 本次候选?.最后回合 ?? null,
    本次进入候选: !是敌方成功开塔 && 行动上下文.是敌方偷塔攻击,
  }
}
...
function 应用敌方成功开塔推断(上次快照, 当前快照, 候选结果) {
  ...
  const 上次敌方确认开塔数 = Number.isInteger(上次快照.敌方.确认开塔数)
    ? 上次快照.敌方.确认开塔数
    : 上次快照.敌方.开塔数
  const 目标敌方开塔数 = 上次敌方确认开塔数 + 候选结果.成功开塔数
...
function 记录敌方开塔日志({
  ...
  const 需要输出日志 =
    判断.敌方偷塔耗兵 > 0 || 候选结果.是敌方成功开塔 || 新增开塔数 > 0
  if (!需要输出日志) return
...
function 确保敌方开塔判断版本() {
  if (状态.敌方开塔判断版本 === 敌方开塔判断版本) return

  状态.敌方开塔候选 = null
  状态.敌方开塔推断数 = 0
  状态.敌方开塔数 = Math.max(状态.敌方开塔确认集合?.size ?? 0, 0)
  状态.敌方开塔调试 = null
  状态.敌方开塔日志列表 = []
  状态.敌方开塔判断版本 = 敌方开塔判断版本
}

function 读取战场快照(数据包) {
  const 回放数据包 = 是网页回放中() ? 同步网页回放塔数据() : null
  同步我方玩家索引()
  const 回合 = 读取当前回合(回放数据包 ?? 数据包)
