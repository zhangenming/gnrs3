...
function 取得敌方开塔判断(上次快照, 当前快照) {
  ...
  const 上次塔差 = 上次快照.敌方.自然塔数 - 上次快照.我方.自然塔数
...
function 更新敌方开塔候选(当前快照, 判断) {
  ...
  const 是敌方成功开塔 = Boolean(旧候选) && 判断.敌方变化.陆地变化 > 0
  const 成功开塔数 = 是敌方成功开塔
    ? Math.max(1, Math.min(判断.敌方变化.陆地变化, 旧候选.攻击次数))
    : 0
  let 当前候选 = 是敌方成功开塔 ? null : 旧候选

  if (!是敌方成功开塔 && 判断.是敌方偷塔攻击) {
    当前候选 = 合并敌方开塔候选(旧候选, 当前快照, 判断)
  }
