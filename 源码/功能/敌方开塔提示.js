...
// 敌方开塔数由塔记忆和兵力差推断共同维护。
// 兵力差变化会先扣除大回合地差、塔差增长和我方开塔耗兵，再判断敌方偷塔耗兵。
//
// 作用范围:
...
export function 更新敌方开塔提示(数据包) {
  ...
  const 推断结果 = 应用敌方成功开塔推断(上次快照, 当前快照, 判断)
...
function 取得敌方开塔判断(上次快照, 当前快照) {
  ...
  const 上次兵力差 = 上次快照.敌方.兵力 - 上次快照.我方.兵力
  const 当前兵力差 = 当前快照.敌方.兵力 - 当前快照.我方.兵力
  const 兵力差变化 = 当前兵力差 - 上次兵力差
  const 上次地差 = 上次快照.敌方.陆地 - 上次快照.我方.陆地
  const 上次塔差 = 上次快照.敌方.塔数 - 上次快照.我方.塔数
  ...
  const 是敌方偷塔攻击 = 敌方偷塔耗兵 > 0
  const 是敌方成功开塔 = 是敌方偷塔攻击 && 敌方变化.陆地变化 > 0
  const 成功开塔数 = 是敌方成功开塔 ? Math.max(1, 敌方变化.陆地变化) : 0
...
function 应用敌方成功开塔推断(上次快照, 当前快照, 判断) {
  const 当前塔数 = 统计塔数()
  if (!判断.是敌方成功开塔 || 判断.成功开塔数 <= 0) {
    return {
      新增推断开塔数: 0,
      推断前塔数: 当前塔数,
      跳过原因: '未满足成功开塔条件',
  ...
  const 目标敌方开塔数 = 上次快照.敌方.开塔数 + 判断.成功开塔数
...
function 记录敌方开塔日志({ 上次快照, 当前快照, 判断, 推断结果, 新增开塔数 }) {
  const 需要输出日志 = 判断.是敌方成功开塔 || 新增开塔数 > 0
  if (!需要输出日志) return
