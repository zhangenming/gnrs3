...
// 扣除后敌方出现中立目标损失时先累积候选，敌方陆地增加时再确认开塔完成。
//
// 作用范围:
...
function 取得敌方开塔判断(上次快照, 当前快照) {
  ...
  const 可疑中立行动 = 我方变化.陆地变化 >= 0 && 敌方变化.陆地变化 >= 0
  const 旧候选 = 取得有效候选(状态.敌方开塔候选, 当前快照.回合)
  const 本次可累积损失 = 可疑中立行动 && 敌方变化.净损失 > 0
  const 本次可直接完成候选 = 本次可累积损失 && 敌方变化.陆地变化 > 0
  const 本次可开始候选 = 本次可累积损失 && 敌方变化.净损失 >= 开塔最小净损失
  const 本次可写入候选 = Boolean(旧候选) || 本次可开始候选 || 本次可直接完成候选
  const 本次候选 =
    本次可累积损失 && 本次可写入候选
      ? 合并候选(旧候选, 当前快照.回合, 敌方变化.净损失)
      : 旧候选
  const 是敌方完成开塔 =
    可疑中立行动 &&
    敌方变化.陆地变化 > 0 &&
    Number.isInteger(本次候选?.累计损失) &&
    本次候选.累计损失 >= 开塔完成最小净损失
  const 是敌方开塔 = Boolean(
    本次可开始候选 || 本次可直接完成候选 || 是敌方完成开塔,
  )
  状态.敌方开塔候选 = 是敌方完成开塔 ? null : 本次候选
  ...
  function 取得有效候选(候选, 当前回合) {
    if (!候选) return null
    if (!Number.isInteger(当前回合)) return null
    if (!Number.isInteger(候选.开始回合)) return null
    if (!Number.isInteger(候选.最后损失回合)) return null
    if (!Number.isInteger(候选.累计损失) || 候选.累计损失 <= 0) return null

    const 间隔 = 当前回合 - 候选.最后损失回合
    const 年龄 = 当前回合 - 候选.开始回合
    if (间隔 > 开塔候选最大间隔 || 年龄 > 开塔候选最大年龄) return null

    return { ...候选 }
  }

  function 合并候选(候选, 当前回合, 净损失) {
    if (!Number.isInteger(净损失) || 净损失 <= 0) return 候选
